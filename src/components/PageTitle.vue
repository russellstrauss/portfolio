<template>
	<header class="page-title">
		<button v-if="showBack" class="back-button" type="button" @click="goBack" aria-label="Go back">
			<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 108.42 1280 1063.16" aria-hidden="true" focusable="false">
				<g>
					<path d="M84.859,738.095l-1.954-0.428h837.055l-263.14,263.72c-12.886,12.875-19.954,30.316-19.954,48.622 c0,18.307,7.068,35.627,19.954,48.532l40.944,40.965c12.875,12.875,30.032,19.994,48.328,19.994 c18.306,0,35.473-7.068,48.348-19.943l451.071-451.03c12.926-12.927,19.994-30.145,19.943-48.46 c0.051-18.417-7.018-35.646-19.943-48.552L794.441,140.443c-12.875-12.865-30.031-19.943-48.348-19.943 c-18.296,0-35.453,7.089-48.328,19.943l-40.944,40.966c-12.886,12.854-19.954,30.021-19.954,48.327 c0,18.296,7.068,34.558,19.954,47.423L922.93,542.353H83.924c-37.701,0-69.38,32.494-69.38,70.173v57.939 C14.543,708.144,47.159,738.095,84.859,738.095z"/>
				</g>
			</svg>
		</button>
		<h1>{{ title }}</h1>
	</header>
</template>

<script>
	export default {
		name: 'PageTitle',
		
		props: {
			title: {
				type: String,
				required: true
			},
			showBack: {
				type: Boolean,
				default: true
			}
		},

		methods: {
			goBack() {
				if (this.$router) {
					// Prefer router history if available
					if (window.history.length > 1) {
						this.$router.back();
					} else {
						this.$router.push('/');
					}
				}
			}
		}
	}
</script>

<style lang="scss">
	@use '@/sass/mixins' as *;
	
	.page-title {
		margin-bottom: 80px;
		display: flex;
		align-items: center;
		gap: 16px;
		flex-wrap: wrap;
		
		h1 {
			@include heading-font;
			font-size: 32px;
			margin-bottom: 0;
			flex: 1 1 auto;
			min-width: 0; // allow text to wrap instead of forcing overflow
		}

		.back-button {
			display: inline-flex;
			align-items: center;
			justify-content: center;
			width: 40px;
			height: 40px;
			background: transparent;
			cursor: pointer;
			flex: 0 0 auto;
			border: none;
			
			svg {
				width: 100%;
				height: 100%;
				fill: white;
				transform: rotate(180deg);
			}

			&:hover {
				
				svg {
					fill: $hover-color;
				}
			}
		}
	}
</style>

